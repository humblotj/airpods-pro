...
const Hero = () => {
  ...
  const move = (index: number, start: number, immediateRender: boolean = true) => {
    animateFromTo(`.hero-bullet-points._${index}`, { y: 40 }, { y: -40 }, start, 20, immediateRender);
  };

  const opacity = (index: number, start: number, to: number, immediateRender: boolean = true) => {
    animateTo(`.hero-bullet-points._${index}`, { opacity: to }, start, 5, immediateRender);
  };
  ...
  useEffect(() => {
    ...
    animateTo('.lottie-animation-1', { scale: 0.85 }, 20, 80);

    animateHeroBullet(2, 40);
    animateHeroBullet(3, 60);
    animateHeroBullet(4, 80);

    animateTo('.lottie-animation-1', { opacity: 0 }, 97, 3, false);
  }, []);
